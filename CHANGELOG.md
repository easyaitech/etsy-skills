...
- `orders-customers/references`：**平台 preset 按 `platforms/` 分组，对齐 `listing-catalog` 的跨 skill 约定**。`references/xiaohongshu-orders.md` → `references/platforms/xiaohongshu.md`（去掉 `-orders` 后缀，命名对齐 `platforms/<平台>.md`）。Etsy 仍是内置 inline preset，无独立文件。全仓 6 处指针更新到新路径（`orders-customers/SKILL.md` ×3、`references/base-schema.md`、`references/order-handling.md`、`shared/platform-config.md`、`shop-foundation/assets/COMMERCE_PLATFORM_template.md`）；grep 验证无旧文件名残留死链。**纯路径约定对齐，不改语义内容**——多平台架构本身（PR #49）已落地。
