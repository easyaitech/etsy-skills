...
### 新增
- `logistics-tracking`：新增**跨境物流状态自动跟踪**公共引擎 skill——用 17TRACK 聚合查询把店铺总 Base `Orders 订单` 表里"已发货未签收"的订单一路跟到签收，状态跃迁主动推飞书给运营者，签收交回 `orders-customers` 的签收评价引导 SOP。三模式：A 接入（给 Orders 补物流字段 + 建 `物流巡检运营` 中央表 + 配 17TRACK key）/ B 手动查单（"这单到哪了"）/ C 每日巡检（register-once + standard 轮询；v1 手动触发但保留推送，v2 才接 Hermes 无人值守 cron）。多租户公共引擎：一个集中管理的 17TRACK key、按 key 维度限速串行节流 + 429 退避、配额护栏读 `/getquota` 实时 `quota_remain`（批内重查 + `单轮注册上限`，防一次 sweep 烧光共享额度）；register 幂等不重扣（实测码 `-18019901`）、空注册状态视同未注册以纳入接入前存量单、TTL 用 `物流注册时间` 兜底无事件黑洞件。单写者边界：本 skill 只写 `物流*` 字段，`状态` / SOP 归 `orders-customers`，`快递公司` 只读。经 `/plan-eng-review`（13 决策 + Codex outside voice）+ 17TRACK API v2.2 实测 spike + `/review`（Codex 跨模型 9 处缺口全修）评审定稿。配套：注册进 [`etsy-stack.json`](etsy-stack.json)、[`orders-customers/references/base-schema.md`](orders-customers/references/base-schema.md) 加物流字段衔接。**注**：v1 引擎 docs 已落，**未在 live Base 跑过、未部署到 Hermes**（接入需在运行机 `lark-cli auth login` + 配 17TRACK key）。
