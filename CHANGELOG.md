...
- **Pinterest 文案 SEO 规则大修（pin-composition.md）**，依据 2026-08-21 对 FuBlessings 已发 25 条 pin 的全量复盘 + Pinterest 官方/行业最佳实践：
  - 标题新增「前 40 字符定生死」前置规则（feed 只展示约前 40 字符）：品类词/高意图词必须进前 40 字符，汉字与拼音一律放后半段；明令禁止 `The Meaning of {汉字} ({拼音})` 博客书名腔（生产漂移实锤：近期十余条 pin 全长成这样，其中含义检索零命中）。
  - 标题模板拆成双意图族：A 礼物/购买意图（原有 4 维组词保留）、B 含义/灵感检索意图（新增——旧版只有礼物模板，含义型 pin 没有 sanctioned 格式正是 agent 自创博客腔的根因）。
  - 描述改为「前 50-60 字符是 feed 唯一可见部分」前置规则，禁止汉字/拼音/定义式开场；建议长度收紧到 200-400 字符（35-55 词，BRAND 更严从其严），明确硬上限 800 是软截断兜底不是写作目标；给出三句骨架（场景钩子→轻含义→收礼人/软引导）；「不写百科」升级为带真实漂移反例的硬规则。
  - hashtag 数量两处矛盾（3-5 vs 4-6）统一为 3-5，标注 2026 起 hashtag 检索权重很低、正文关键词才是主力，但也不许一个不放（生产漂移：多数已发 pin hashtag 全缺）。
  - 新增「排期」节：节日/场景型 pin 提前 6-8 周入队（Pinterest 分发慢热）。
  - 自检清单新增 SEO 六项（前 40/前 60 字符、博客腔、百科长文、hashtag、图内文字保真），每项都对应一条生产真实失守记录。
  - 输入清单：创意主题的推断来源从「素材文件名」改为素材池 `图内文字` 列，禁止按文件名拼音猜图上文字（对齐主仓 enqueue 逐字比对硬闸）。
  - 渠道沉淀区落下首两条记录（此前一直为空）。
...
- **`etsy_order_sop_get` 放行 `sendCard`**（配套主仓 v0.6.54.0「订单 SOP 可见性」）：店主说
  「XX 那单到哪一步了/流程列给我看看」时，get 带 `sendCard: true`，后端会把该单的 SOP 流程
  清单卡片（含打勾按钮）直接重新发进群并返回 `data.cardsSent`——agent 回一句「卡片已发到群里」
  即可，不用逐条复述步骤。不带该参数行为不变；后端只认严格布尔 `true`。
...
- **店长生成的文件终于交得出去了**（2026-08-09 店主实测）：店长按技能生成了一封 PDF 信，
  然后只能说「文件在我本地」——后端此前**只有下行没有上行**，没有任何接口能把 agent 手上的
  字节写进云盘（`bitable/attachment-upload` 只收「系统发给 agent 的那张签名图片」）。于是全局
  红线「不要把本地路径发给租客」与「东西只在本地」直接对撞，店主什么都拿不到。
  - 新增 `scripts/drive_upload_tool.py`（装为 `~/.local/bin/upload_to_drive`）：
    `upload_to_drive --folder 输出 --file <本地绝对路径> [--name 上传后的名字]`，
    走后端 `/api/hermes/drive/upload-file` 传成**云盘目录里的真实文件**，回一条可发给店主的链接。
  - 直接让 agent 用 curl 拼内嵌几 MB base64 的 JSON 太脆（引号 / 折行 / 体积），所以封装成命令。
  - 失败时回 `ok:false` 且带 `uploaded:false`——那意味着文件**没进云盘**，如实说没传成，
    别报成已上传，也别退回去给本地路径。配套后端 >= v0.6.44.0。
...
- **订单 SOP 三工具进包装层白名单**（配套后端 v0.6.31.0，docs/order-sop-plan.md P3）：
  ...
  - `etsy_order_sop_update`（orderNumber + stepId/stepName 二选一 + status=done/undo，
    进度唯一的 agent 写口，写操作 5xx 按 mutation 语义 `safe_to_retry=False`）；
...
- `etsy_listings_get` 新增可选 `acceptCachedWithinMs`（毫秒）：声明「这次问题能接受多旧的
  数据」。后端已把每次读到的 listing 存下来，全都有够新的已存结果时直接给，不再真的去爬。
  **不传 = 照常真读，默认行为一个字不变。**
  - 为什么由调用方声明：listing 是平台的快照、不是店主的账本，「现在什么价」和「我上次
    写的标题是啥」对新鲜度的要求差着数量级，只有发起调用的人知道是哪一种。
  - 省的是真金白银：公开读最长等 320 秒；后台读条目间随机等 3~8 秒且滚动 24 小时只有
    30 条配额，重复爬同一个 listing 就是从配额里再买一遍。
  - `scope=shop` 传了也不转发（整店是枚举，证明不了「这就是全店」）；拿到
    `source.fromCache=true` 的结果只能说「几分钟前看到的是…」，不能说「现在是…」。
  - 需后端 >= v0.6.17.41。
...
- **后端 API 访问约定升为 canonical：新增 [`shared/backend-api-access.md`](shared/backend-api-access.md)（收编 Mac mini 上 agent 自建的同名文档，逐条核对后**大改**）**。**修的问题**：`YANGGEDIANZHANG_API_BASE` / `YANGGEDIANZHANG_HERMES_TOOL_TOKEN` / `YANGGEDIANZHANG_TENANT_ID` 这三个变量名与用法，此前在本仓 grep **零命中**——所有 skill 的鉴权段只写「用按租户派生的 Hermes 工具令牌」，从不说变量叫什么、在哪能拿到、怎么调。bot 能调通后端，靠的是一份 agent 自己写的、只活在 fublessings 那台机器上的说明书（机器重装 / profile 重 provision 即丢，同款漏损见上游热修条目）。现固化进 `shared/`，因为它是全 skill 共用的**调用约定**而非某个 skill 专有，并与 [`shared/tools-architecture.md`](shared/tools-architecture.md) §接口形态 第 4 点（「agent 调工具的真实路径 = Hermes skill → 调 ECS 的 HTTP endpoint」）互相链接——那条一直缺这块落地细节。`orders-customers` 的 `etsy-order-message-tool.md` / `etsy-reply-draft-tool.md` 鉴权段加指针过去。
  - **收编时逐条核对，agent 原文的机制解释几乎全错，已按实测 / 源码改正**（不照抄）：① **「execute_code 拿不到 env、只有 terminal 能拿」——错**。`env_passthrough` 是喂**两个**沙箱的白名单（`tools/env_passthrough.py` 明写「execute_code, terminal」，`code_execution_tool._scrub_child_env` 第一条规则即放行）；实跑该函数，三个变量全部存活（对照组 `SOME_OTHER_TOKEN` 被剔除，证明剔除逻辑在跑）。真实历史是那段时间 plist 漏配了三个变量（注入链路第 1 段断了），**两个**沙箱都拿不到，agent 据此倒推出一个不存在的机制。② **「API server 建立的 session 可能拿不到」——删**（2026-07-15 实测飞书对话那条路的 terminal 拿得到）。③ **lark-cli 的 `HOME` 坑方向写反了**：agent 说 execute_code 要覆盖、terminal 不用；实测 `terminal` 拿到的真实 HOME 反而报「未配置」，只有 profile home 能出 `identity: bot`——`home_mode: auto` 在主机安装上给**两个**沙箱的都是真实用户 HOME，故**两边都要覆盖**。④ **飞书系统签名图片「只能看一次 / 换上下文就 403 / 无法下载」——错**：实现是 HMAC + `expiresAt`、**有效期 7 天**、无 IP 绑定、无一次性限制，有效期内任何上下文都能重复取并下载；后端注入的系统提示本身就写着「先下载/打开这些链接，不要再要求客户重新上传」，agent 的说法与之直接冲突，会让 bot 白白拒绝它做得到的事。⑤ **「飞书 Base 权限要去管理后台两步手配（开 `bitable:app` + 手动加协作者）」——删**：Base 权限走管理员侧运维端点（bot 自建自持开通 / 一键授权 / 协作者对账巡检），不是让 agent 教用户去后台手点；改为「不是你的活，报管理员」。
  - **保留并强化了 2026-07-16 清理时定的规范**：只写**判据**、不列失败响应样例（避免样例被 bot 复读成实测结果——实锤见「bot 两次谎报错误码实为编造」）。新增**令牌只按引用使用**的硬性写法（命令里写 `$VAR` 让 shell 展开，agent 自始至终看不到值；自查只看 `${#VAR}` 长度不看值），把各 skill 鉴权段那句「不手写 / 不读取 / 不回显」落到可执行。
