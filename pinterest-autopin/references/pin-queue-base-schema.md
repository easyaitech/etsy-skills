...
> Pinterest pin 不再单独建表。一条 Pinterest pin = 店铺总 Base 内 `社媒发布队列` 表里 `平台 = Pinterest` 的一行。默认只补齐最小可运行字段；跨平台通用字段（`任务 ID` / `平台` / `状态` / `关联素材` / `计划发布时间` / `自动发布` / `发布适配器` / `执行锁` 等）见 [`publish-composer/references/base-schema.md`](../../publish-composer/references/base-schema.md) 和 [`social-publisher/references/publishing-queue-contract.md`](../../social-publisher/references/publishing-queue-contract.md)。不要为 Pinterest 队列默认创建独立 Base / 独立表；迁移期旧的独立 Pinterest 队列数据源只作为只读来源。

## 数据模型

一条 `平台 = Pinterest` 的记录就是一个 Pinterest Pin。单图 pin 和轮播 pin 共用同一行：

- 单图：`关联素材` 只有 1 项，`发布类型 = 单图`
- 轮播：`关联素材` 有 2-5 项，`发布类型 = 多图轮播`
- 多张图不要拆成多条记录；它们属于同一行里的有序图片列表
- 发布成功后只回写这一行的一个 `发布 URL`
- 发布顺序以 `关联素材` 中列出的顺序为准；若历史行已有 `素材顺序`，可作为人工追溯，不作为新建必需列
...
4. `Board (Pinterest)` / `Alt Text (EN)` 作为 Pinterest 专属列保留；旧 `发布素材` / `image 路径` 只作为迁移期只读证据
...
`社媒发布队列` 通用字段之外，Pinterest 行默认只需要下面这些列（非 Pinterest 行留空即可）。

> `Board (Pinterest)` 与 `Alt Text (EN)` 对应 PinterestExt 的 `board_id` / `alt_text`。不要默认创建 `发布素材`、`image 路径`、`图片数量`、`封面图`、`创意主题`、`平台字段 JSON` 或 `平台扩展 (typed)`；这些只有真实发布器已经读取时才补。
...
| `关联素材` | 关联 `Asset Variants 派生素材` 表的 Pinterest 规格变体（**非 canonical 原图**），**允许多值**；单图关联 1 条，轮播关联 2-5 条；发布顺序以该字段中列出的顺序为准 |
...
| `外部队列 ID` / `ECS job ID` | 后端 publish-service / ECS dispatch 返回的 `jobId`。当前运行表常用 `外部队列 ID`；新表只需二选一，不要两列都建 |
...
默认使用 `关联素材` 存放所有待发布图片的变体引用，每行一个，顺序就是 Pinterest carousel 展示顺序：
...
`Alt Text (EN)` 字段存放每张图的独立 alt text，用 `---` 独占一行分隔。段落顺序与 `关联素材` 的图片顺序一一对应：
...
写入前列出字段值清单给用户确认，确认后才用 `lark-base` 写入。草稿组好进 `待审`；`外部队列 ID` / `ECS job ID` 在模式 C 创建 job 时写，`发布 URL` / `发布时间` / `失败原因分类` / `失败原因` 等在模式 C 才填。

多图 pin 时额外展示：
- 图片数量和顺序（编号列表，来自 `关联素材`）
- 封面图（第一张，只展示给用户确认，不默认建字段）
- 每张图的 alt text 对照
- `发布类型 = 多图轮播` 自动设置
...
| `关联素材` 每行都能被服务器解析为授权下载 asset | 中止，先补服务器 asset 流程 |
| 发布副本有 AI metadata / watermark 清理证据 | 中止，先按 `image-processing.md` 生成发布副本 |
