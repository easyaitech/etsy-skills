...
| `Tags` | 多行文本 / 单行文本 | Etsy 13 个 tag 槽，直接保存为英文文本；不同 tag 之间用半角逗号 `,` 分隔，方便复制到 Etsy 后台 |
| `Materials` | 多行文本 / 单行文本 | Etsy 13 个 material 槽，直接保存为英文文本；不同 material 之间用半角逗号 `,` 分隔，方便复制到 Etsy 后台 |
| `Sustainability` | 多选 | Etsy 后台 Sustainability 属性；最多选 3 个。只在材料/方法真实符合 Etsy 定义时填写，不确定留空 |
| `Occasion` | 单选 | Etsy 后台 Occasion 属性；只在商品明确服务某一场景时填写，不要把“适合作礼物”泛化成场景 |
| `Holiday` | 单选 | Etsy 后台 Holiday 属性；只在商品图案/文案/用途明确关联某个节日时填写，不确定留空 |
| `视频链接` | 文本 / 链接 | Etsy listing 视频（5–15 秒，1 条）的飞书云盘链接。**店铺硬性约束：每条 listing 必须有视频**，未拍即视为未完工，状态停在"草稿"。原始视频走 `assets-library`，本字段存素材库链接或外链；不再单独维护视频附件列 |
| `售价 (HKD)` | 数字（货币） | 主售价；变体定价用变体表。FuBlessings 商品 Base 主售价统一用港币，与 Etsy 后台/地区显示价格分开记录 |
