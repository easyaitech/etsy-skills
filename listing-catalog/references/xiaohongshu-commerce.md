...
本文件是小红书平台 preset。仅在目标销售平台是“小红书 / 小红书店铺 / RED”时读取；不要把这些字段套给 Etsy 或其他平台。

依据：
- 小红书大学开放平台商品结构：SPU / SPL / SPV / ITEM 分层。来源：https://school.xiaohongshu.com/en/open/product/product-structure.html
- 小红书大学开放平台 Create SPU：创建商品时包含品牌、末级类目、商品名、规格、商品图、描述、商品特色、产品参数、FAQ、使用指南、图文详情、条码、重量、原产国、价格等字段。来源：https://school.xiaohongshu.com/en/open/product/create-spu.html
- 小红书大学开放平台 Submit Review：Open API 导入的 SPU / SPV 信息自动审核通过，创意编辑 / 文描信息需要提交审核。来源：https://school.xiaohongshu.com/en/open/product/spl-item-submit.html
- 小红书 API 商品基础介绍：商品 ID、SKU、SKU 状态、价格、库存、上下架状态都是商品管理核心信息。来源：https://xiaohongshu.apifox.cn/doc-2810949
...
当 `COMMERCE_PLATFORM.md` 包含小红书平台时，`listing-catalog` 模式 A 建 `Products 商品` 表必须在通用核心字段外加建 `base-schema.md` 的“小红书字段”分组。字段多，但这是平台约束；可以先建空字段，后续按 SKU 慢慢补齐。
