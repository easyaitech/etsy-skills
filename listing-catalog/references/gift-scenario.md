...
> **⛔ 平台门槛：本环节是 Etsy 平台专属，非 Etsy 平台整段跳过。**
> 礼物 / 节日维度是 Etsy 搜索流量的大头、是 Etsy SEO 的输入模块，其产出（礼物词库 → title 礼物维度槽 / 13 tags 礼物槽 / description 段 3 礼物语境）全部绑定 Etsy 的 listing 结构。小红书走内容种草模型、其他平台各按自己的 `COMMERCE_PLATFORM.md` 规则，都**没有**对应的礼物槽位，所以本环节**不套用到非 Etsy 平台**。本文件只在目标平台是 Etsy 时读取（注册见 [`../../shared/platform-config.md`](../../shared/platform-config.md) § 内置平台 preset · Etsy）。
>
...
>
> **进入条件见 SKILL.md 模式 B step 5.5**。Etsy 平台每条 listing 都跑（< $20 SKU 走轻问法分支）；非 Etsy 平台不跑。
...
**为什么 Etsy 下强制**：现有流程把礼物维度当可选 → 80% listing 不会有礼物词 → 漏吃 Etsy 礼物搜索流量。礼物维度提升到输入级，与 BRAND.md / SHOP.md / eRank 的「输入 → 词库 → 文案」范式同构。（强制只对 Etsy 成立——非 Etsy 平台无此环节，见本文件顶部平台门槛。）
...
- **Etsy 平台每条 listing 5.5 都要跑一遍**——批量上新时不能跳过；如果用户在批量场景下嫌烦，建议先按"主推 SKU 走完整 / 长尾 SKU 走轻问法"分批跑，**而不是**整批跳过。**非 Etsy 平台（小红书等）整段不跑**——这是平台门槛，不是可省略的麻烦
