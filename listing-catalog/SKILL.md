...
4. 按 schema 逐字段配置（核心字段必建；小红书字段在启用小红书平台时必建；其他辅助字段视用户需要），并按下方“默认视图字段”设置默认 Grid View
5. `Products 商品` 表必须保留原 SKU 字段作为业务主键；如需要内部稳定 ID，新增 `内部 ID` 字段，不要因迁移重命名 SKU
6. 落盘后告诉用户 Base 链接 + 表清单 + 字段清单 + 默认视图设置 + 可选的下一步（"导入第一条 SKU 试试")
...
4. 按平台选择规则来源：
   ...
   - **小红书**：读 `references/platforms/xiaohongshu.md`，理解 SPU / SPL / SPV / ITEM 分层、商品图、描述、特色、属性、FAQ、使用指南、图文详情、条码、重量、原产国、价格、库存、上下架和审核状态字段
