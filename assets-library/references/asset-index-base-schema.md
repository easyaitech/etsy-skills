...
2. **物理薄、语义厚**：文件夹只做来源归属粗分（商品 / 品牌 / 客户 / 工作室 / 待处理），"属于哪些 SKU / 渠道 / 用途 / 阶段"全在 Base 字段
...
| 决策 | 在哪做 |
|---|---|
| 这个文件物理放云空间哪条路径 | 文件夹（[folder-structure.md](folder-structure.md) + [asset-types.md](asset-types.md)）——5 个一级文件夹按来源归属 |
...
文件夹只做来源归属的粗分（商品 / 品牌 / 客户 / 工作室 / 待处理），确保每个文件**有且只有一份**。所有细粒度分类（SKU / 渠道 / 用途 / 阶段）交给 Base。进 Base 即为承诺"这是用过 / 准备用的素材"。
