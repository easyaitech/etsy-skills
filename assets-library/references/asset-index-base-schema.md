...
> 资产库的**语义层**——一行一个素材，多维标签 + 关联其他 Base，把"网状归类"压在 Base 里。
...
2. **物理薄、语义厚**：文件夹只做粗分（原料 / 成品 / 受限），"属于哪些 SKU / 渠道 / 用途"全在 Base 字段
3. **关联优于多选**：能跟其他 Base 关联的（SKU / 订单 / 客户）走关联字段；只是分类标签的（用途 / 渠道）走多选字段
4. **录入即合规**：录入同时强制带 BRAND 合规自检，避免事后补登
...
| 素材类型 | 多选 | ✓ | `摄影原图 / 摄影成图 / 场景图 / 视频原料 / 视频母版 / 视觉模板 / Logo / 字体 / 包装物料 / 客户拍摄 / 客户定制参考` | 多选——一个素材可同时是"摄影成图"+"客户拍摄" |
...
|---|---|---|
| 全部素材 | 默认 | 按"创建时间"倒序 |
| 按 SKU 分组 | 给某 SKU 找所有相关素材 | 分组：关联 SKU |
| 待整理 | 录入不全的素材，定期补 | 筛：关联 SKU IS EMPTY OR BRAND 合规 IS EMPTY |
...
| Pinterest 候选 | 待发 Pinterest | 筛：用途标签 ⊇ Pinterest |
| Instagram Reels 候选 | 待发 Reels | 筛：用途标签 ⊇ Instagram Reels |
| 视频库 | 仅看视频类 | 筛：素材类型 ⊇ 视频原料 / 视频母版 |

## 录入约定（模式 B 归档时执行）

1. 一次只录一行；**不要**给一个目录建一行抽象索引
2. 必填字段优先录全：素材标题、文件链接、文件名、素材类型、公开授权、BRAND 合规
3. 关联字段按素材类型条件触发：
   - 摄影原图 / 摄影成图 / 场景图 / 视频原料 / 视频母版 → 关联 SKU 必填（跨 SKU 的 vlog 等可空）
   - 客户拍摄 / 客户定制参考 → 关联订单 + 关联客户必填
4. 用途标签可以**先空着**——发布到某渠道时再勾上（避免"Pinterest 候选"视图被未发的素材污染）
...
| 这个文件属于哪些 SKU / 客户 / 用途 / 渠道 | 素材索引 Base（本文档） |
| 怎么找一类素材 | 素材索引 Base 的视图 |

文件夹不再追求"完美归类"——确保每个文件**有且只有一份**放在合理的粗分目录下即可，网状关系交给 Base。
