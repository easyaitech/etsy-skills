...
| # | 输入 | 是否必需 | 缺失时怎么办 |
|---|---|---|---|
| 1 | **`Products 商品` 表中该 SKU 行** | 必需 | SKU 不在店铺总 Base：**阻塞** + 提示用户先回 listing-catalog 模式 A 建一行最小记录（至少 title + 品类），再回模式 D。本 skill 不偷偷建 SKU 行 |
...
加："+ `Products 商品` 表（lark-base，模式 D 读 SKU 行）"
