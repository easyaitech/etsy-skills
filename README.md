...
| [`listing-catalog`](listing-catalog/SKILL.md) | 飞书 Base 商品目录 + 按目标电商平台配置撰写商品页 / listing 文案 |
| [`orders-customers`](orders-customers/SKILL.md) | 飞书 Base × 2（订单 + 客户）+ 按平台配置支撑客服/履约 SOP + 客户标签 |
...
| [`pinterest-autopin`](pinterest-autopin/SKILL.md) | Pin Queue Base + 调用本地 Pinterest-autopin 工具发 pin（用 `ecommerce-stack pinterest-tool status/update` 管理，旧命令兼容） |
