...
- 安装十二个 Agent 工具（清单由 `etsy-stack.json` 驱动）：`etsy_listings_get`、`etsy_customer_messages_get`、
  ...
  `etsy_order_sop_update`、`etsy_order_sop_propose_flow_change`、`etsy_order_shipment_submit`
...
十二个 Etsy 工具只接收业务 JSON，不接收 tenant、token 或内部轮询 ID；统一最终结构见
