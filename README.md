...
```bash
curl -fsSL https://raw.githubusercontent.com/easyaitech/etsy-skills/v1.0.9/install.sh | bash
```
...
- 安装七个 Agent 工具：`etsy_listings_get`、`etsy_customer_messages_get`、
  `etsy_customer_messages_publish`、`get_etsy_orders`、`get_etsy_stats`、
  `describe_etsy_stats`、`summarize_etsy_stats`

七个 Etsy 工具只接收业务 JSON，不接收 tenant、token 或内部轮询 ID；统一最终结构见
[`shared/etsy-agent-tools.md`](shared/etsy-agent-tools.md)。

谨慎模式（先看再跑）：

```bash
curl -fsSL https://raw.githubusercontent.com/easyaitech/etsy-skills/v1.0.9/install.sh -o install.sh
...
偏好存在服务端、与 skill 引擎物理分离，`ecommerce-stack update` 只换引擎——**升级零冲突、零 merge**。旧的「工作区 `skill-prefs/` 文件覆盖层」已废除（从未在任何租户使用）；`ecommerce-stack doctor` 会检测残留目录并提示清理。
...
| `ECOMMERCE_SKILLS_REF` | `main`（推荐传具体 tag，例如 `v1.0.9`） |
...
│   ├── preamble.md            # 版本检查 / 工作区解析 / 客户偏好 / 写入约束 / 技能目录写入禁令 / 工作语言 / 经营原则
