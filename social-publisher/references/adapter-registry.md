...
|---|---|---|---|---|---|
| Pinterest | enabled | 单图 / 多图轮播 | `pinterest-autopin` | Pinterest-autopin 工具可用、Chrome 登录态可用、Pin Queue 可用 | 当前唯一真实自动发布适配器。发布过程必须走 validate → test → final，并回写 Publishing Queue。 |
...
- 单图和多图轮播都先映射到 Pin Queue，再由 `pinterest-autopin` 发布。
- `外部队列 ID` 保存 Pin Queue 的 `pin_id`。
