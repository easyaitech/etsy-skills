...
description: 社交媒体自动发布总控层：从 publish-composer 的跨平台 社媒发布队列 读取待发布任务，按平台适配器执行发布、定时自动发布巡检、失败重试和结果回写。当前真实发布适配器只有 Pinterest（pinterest-autopin，经 yanggedianzhang 服务器 + 浏览器插件执行）；小红书 adapter（xiaohongshu-autopost）后端 + 契约已就绪但 **staged 未对外开放**（只草稿 + 人工对账，不跑真发）；Instagram、TikTok 等 planned/manual-only。能建任务和人工对账，但未 enabled 的平台不能声称已自动发布。用于用户说“自动发布社媒 / 到点发布 / 跑发布队列 / 发 Pinterest / 对账发布结果”等场景。
---
