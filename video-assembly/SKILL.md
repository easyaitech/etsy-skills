...
> **工具架构**（见 [`shared/tools-architecture.md`](../shared/tools-architecture.md)）：本地 FFmpeg 是纯本地媒体计算——无平台、无登录、无密钥、无租户身份，落在约束热区之外，当前形态已符合约束。仅当将来要无人值守 / 上规模渲染时，才考虑把重计算迁 ECS。
