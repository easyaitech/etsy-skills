...
**对外的实操接口**：
- 飞书 Base（用 `lark-base` skill 操作店铺总 Base 内 clips / video_jobs 表 + 反查 `Products 商品` / `SKUs 变体` 表）
...
| `Products 商品` / `SKUs 变体` 表（listing-catalog）| `product_id` / 标题 / 核心卖点 | 筛选片段 + 生成 overlay 文案 |
...
- **listing-catalog**：读 `Products 商品` / `SKUs 变体` 表取产品信息（product_id / 标题 / 卖点）
