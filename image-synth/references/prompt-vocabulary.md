...
### 模式 A：按 Etsy 槽位查 shoot-brief.md §C

槽位 ID 取值见 [`assets-library/references/etsy-listing-photo-slots.md § 3`](../../assets-library/references/etsy-listing-photo-slots.md#3-槽位-id-与素材索引-base-用途标签-字段对齐)（contract source）。每个槽位 ID 对应 shoot-brief.md §C 同名段；映射规则：
