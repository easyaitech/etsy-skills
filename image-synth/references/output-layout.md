...
- `{标签ID}`：模式 A 用目标平台商品图用途。Etsy 用槽位 ID（取值见 [`assets-library/references/etsy-listing-photo-slots.md § 3`](../../assets-library/references/etsy-listing-photo-slots.md#3-槽位-id-与-assets-素材池-表-用途标签-字段对齐)）；小红书取值（`商品图` / `使用指南图` / `图文详情图`）已封存（产品决策 2026-07-24：专注 Etsy），仅未来解封后启用。模式 B 用渠道值（取值见 [`assets-library/references/asset-index-base-schema.md` § 用途标签](../../assets-library/references/asset-index-base-schema.md) 渠道段，如 `Pinterest` / `Instagram Posts` / `Instagram Reels` / `小红书` / 等）。**直接用词汇表原值**（含中文 / 空格也无所谓——本地 fs 都接受），不做小写化转写。
- `{SKU}`：`Products 商品` 表 SKU；用户没指定 SKU（如做品牌物料图）用 `general`
