...
4. `平台扩展 (typed)` 是合法 `XiaohongshuExt`（`note_type` / `topic_tags` / `cover_caption`），过 validator。
5. 商品型笔记 `链接` 来自 `Products 商品` 表 `分享链接`（缺则留空标待补，不拼 URL）。
6. `关联素材` 指向 `Asset Variants 派生素材` 的小红书规格变体（3:4 封面 / 商品图），`公开授权 = 已授权`、AI 清理已记录；`封面素材` + 图序明确。
...
把 `jobId` 写入 `社媒发布队列` 的 `ECS job ID`，状态改成 `发布中`（test 阶段）/ 备注 `待测试确认`。
