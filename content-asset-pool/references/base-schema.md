...
| `外部队列 ID` | 单行文本 | 平台子队列 ID；Pinterest 填 Pin Queue 的 `pin_id` |
...
`链接` 必须从商品 Base 的 `分享链接` 字段读取。平台商品 ID 是识别商品的事实，不是拼链接的授权。
...
- 商品型笔记如果没有商品 Base `分享链接`，`链接` 可以为空并标记为待补；不要临时拼小红书商品 URL。
