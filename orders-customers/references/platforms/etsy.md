...
| 11 | 自动化边界 | 不替用户在 Etsy 后台标记发货 / 退款 / 处理 case；本 skill 只产文案 + 维护 Base。发货回填（单号回填 + 承运信息 / 发货确认提交）由用户在 Etsy 后台完成，本 skill 只负责提醒并把它当未完成待办跟到底，见 §发货回填。**站内信有两处放宽**（都必须先在飞书里跟用户逐字定稿，那次确认是唯一人工闸）：① 回复已有会话 → 服务器工具**填进回复框**（只填入、发送仍由用户点击），见 [`../etsy-reply-draft-tool.md`](../etsy-reply-draft-tool.md)；② 主动给某订单的买家发消息 → 服务器工具按**订单号**在订单页定位买家并**代为发送**（真实发出），见 [`../etsy-order-message-tool.md`](../etsy-order-message-tool.md) |
