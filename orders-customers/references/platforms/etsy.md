...
| 11 | 自动化边界 | 不替用户在 Etsy 后台发站内信 / 标记发货 / 退款 / 处理 case；本 skill 只产文案 + 维护 Base。唯一放宽：飞书里定稿的站内信回复可经服务器工具**填进回复框**（只填入、发送仍由用户点击），见 [`../etsy-reply-draft-tool.md`](../etsy-reply-draft-tool.md) |
