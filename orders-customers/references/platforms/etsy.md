...
| 11 | 自动化边界 | 不替用户在 Etsy 后台标记发货 / 退款 / 处理 case；本 skill 只产文案 + 维护 Base。发货回填（单号回填 + 承运信息 / 发货确认提交）由用户在 Etsy 后台完成，本 skill 只负责提醒并把它当未完成待办跟到底，见 §发货回填。站内信是唯一放宽：通过 [`../etsy-message-tools.md`](../etsy-message-tools.md) 获取正式双向文字消息，并在用户明确指定或确认完整正文后真实发送；可按客户 ID、订单号或快递单号定位，不使用旧草稿或旧订单发送工具。 |
