...
本文件是 Etsy 平台 preset（订单 / 履约 / 客服 / 售后）。Etsy 是本 stack 唯一销售平台，处理订单 / 客服 / 履约时直接按本文件执行；不要把这些字段名、语言、Ship-by 规则套给已封存的小红书 preset。契约见 [`platform-presets.md`](platform-presets.md)。
...
| 7 | 消息 / 客服 / 售后入口 | Etsy Messages（站内信）；售后 / 纠纷走 Etsy case / 退款流程 |
| 8 | 消息媒体限制 | 正式消息工具 V1 **只支持文字，不支持图片、附件或视频**；需要媒体时由用户在 Etsy 后台人工处理，不得让工具假装已发送 |
| 9 | 隐私 / 地址 | 完整收货地址、邮箱留在 Etsy 后台 / 打单系统；Base 只保留 `收件地区`（国家 / 州 / 城市级）。**需要完整地址 / 门牌 / 邮编 / 收件电话时用 [`../etsy-order-read.md`](../etsy-order-read.md) 的 `get_etsy_orders` 现查**（`fullAddress` / `phone`）——Base 里只有国家或城市是这条隐私规则的正常结果，不得据此说查不到，也不得让用户自己去 Etsy 后台 Shipping 页看 |
| 10 | 价值标签阈值 | `VIP`：累计 ≥ 3 单 **或** 累计金额 ≥ **USD 300**（若客户表 `累计金额` 使用核算币种，如 CNY，先把 USD 300 按店铺确认汇率折算到同一核算币种再比较）；`品牌相符`：留过 ≥ **50 词英文**实质好评 或 自发社媒分享 |
| 11 | 自动化边界 | 不替用户在 Etsy 后台标记发货 / 退款 / 处理 case；本 skill 只产文案 + 维护 Base。发货回填（单号回填 + 承运信息 / 发货确认提交）由用户在 Etsy 后台完成，本 skill 只负责提醒并把它当未完成待办跟到底，见 §发货回填。站内信是唯一放宽：通过 [`../etsy-message-tools.md`](../etsy-message-tools.md) 获取正式双向文字消息，并在用户明确指定或确认完整正文后提交发送——**提交后还要店主在飞书确认卡上点「确认发送」才真的发出**（`awaiting_confirmation` 一个字都还没发）；可按客户 ID、订单号、快递单号或 Etsy 数字买家 ID 定位（还没下过单的潜在客户走合成身份，读和发都可用），不使用旧草稿或旧订单发送工具。 |
...
对应 `order-fulfillment-sop.md` §阶段 5「发货与物流」的平台后台回填检查项。

- 拿到快递单号后，提醒用户把单号回填到 **Etsy 后台**对应订单，并核对**承运信息 / 发货确认**是否已提交。
- 在 Etsy 后台未填完前，这单仍算有一个未完成待办；发货确认尚未提交时，必须继续作为该单最后一个待办提醒用户，不因 Base 里 `状态 = 已发货` 就当本单收尾。
- 本 skill 不替用户在 Etsy 后台标记发货（见契约 #11）——回填只提醒，动作由用户在后台完成。
