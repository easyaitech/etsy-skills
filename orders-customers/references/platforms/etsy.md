...
| 11 | 自动化边界 | 不替用户在 Etsy 后台标记发货 / 退款 / 处理 case；本 skill 只产文案 + 维护 Base。发货回填（单号回填 + 承运信息 / 发货确认提交）由用户在 Etsy 后台完成，本 skill 只负责提醒并把它当未完成待办跟到底，见 §发货回填。站内信是唯一放宽：通过 [`../etsy-message-tools.md`](../etsy-message-tools.md) 获取正式双向文字消息，并在用户明确指定或确认完整正文后真实发送；可按客户 ID、订单号、快递单号或 Etsy 数字买家 ID 定位（还没下过单的潜在客户走合成身份，读和发都可用），不使用旧草稿或旧订单发送工具。 |
