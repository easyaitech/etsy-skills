...
核心流程负责"对任何平台都一样的部分"；preset 负责"这个平台不一样的部分"。销售平台**固定 Etsy**（产品决策 2026-07-24，契约见 `../../../shared/platform-config.md`）：核心流程直接读 Etsy preset，不做平台路由。
...
销售平台固定 Etsy：直接用 [`etsy.md`](etsy.md)。小红书 preset [`xiaohongshu.md`](xiaohongshu.md) **封存**（产品决策 2026-07-24：专注 Etsy），仅未来解封后启用；封存期间小红书规则不能反向套给 Etsy。其他平台（亚马逊 / Shopify / 淘宝 / 天猫 / 抖店等）没有内置 preset，当前也不接入。

---

## preset 解析（每次任务开始时）

1. 销售平台固定 Etsy：直接读 [`etsy.md`](etsy.md)，无需确认目标平台，也没有其他平台分支。
2. 读到的 preset 优先级**高于**核心流程里的中性默认；但 preset **不得**豁免 `shared/preamble.md` §写入前的通用约束、隐私红线和"不替用户操作平台"边界。
...
| # | 契约项 | 说明 |
|---|---|---|
| 1 | **平台标识 / 别名** | 平台的名字和常见别名（与 Base `销售平台` 字段取值对齐） |
| 2 | **买家语言（默认）** | 面向买家输出的默认语言；店铺级例外沉淀进 `SHOP.md` |
...
| 7 | **买家消息 / 客服 / 售后入口与边界** | 平台消息渠道、售后链路、哪些动作不能代操作；preset 给默认，店铺级例外沉淀进 `SHOP.md` |
...
当前销售平台固定 Etsy，**不为多平台做任何预留设计**（见 `../../../shared/platform-config.md` §未来多平台）；以下仅在未来真要开放新平台时启用。不改**流程逻辑文件**（`SKILL.md` / `order-handling.md` / `order-fulfillment-sop.md`）；`base-schema.md` 是唯一按平台增长的核心文件——只在它的「平台专属字段组 / 平台专属视图」追加内容，通用字段与通用视图不动：

1. 新建 `references/platforms/amazon.md`，按上面 12 项契约写满。
2. 如果亚马逊有值得结构化的订单字段（FBA / MCF、ship-by、买家无邮箱的匿名地址等），在 `base-schema.md` 的「平台专属字段组」追加一个 `亚马逊…` 字段组（暂不细化就先用通用 `平台字段 JSON`）；如该平台有专属异常 / 监控视图（契约 #12），同时在 `base-schema.md` §视图建议追加 `亚马逊异常` 类视图。
3. 不需要改 `SKILL.md` / `order-handling.md` / `order-fulfillment-sop.md`——它们读 preset，不写死平台。
