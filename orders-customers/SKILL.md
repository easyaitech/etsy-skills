...
description: 维护 Etsy 订单 + 客户的两份飞书 Base，并支撑订单处理 / 客服回复 / 客户标签运营。三种触发：(1) 建库：用户提到"建订单库 / 客户库 / 订单表 / 客户表"等首次建立请求时——按推荐 schema 建两个 Base；(2) 录入/更新：用户提到"录订单 / 收到一个新订单 / 这个客户回头买了 / 给某客户加备注"等订单与客户事件时——通过 lark-base 写入或更新；(3) 客服 + 标签运营：用户要"回客户消息 / 处理差评 / 处理退货 / 给某类客户群发 / 标记 VIP / 找回头客"等客服或标签查询任务时——按客服 SOP + BRAND.md 文案语调输出回复，并维护客户标签。所有客服文案严格遵守 BRAND.md 文案语调原则与 SHOP.md 政策口径。
---
...
**对外的实操接口**：飞书 Base（用 `lark-base` skill 操作）+ 项目根目录的 BRAND.md / SHOP.md（用 `shop-foundation` 维护）。
...
| 来源 | 提供什么 | 怎么用 |
|---|---|---|
| `./BRAND.md` | 文案语调 / 客服姿态 / 边界 | 写客服回复时严格遵守"应该说"、"避免说"、"原则"段（特别是处理差评的姿态原则） |
| `./SHOP.md` | 处理时间 / 退换货 / 运输 / 定制政策 | 客服回复涉及承诺时引用 SHOP.md 原文，**绝不自编**（避免承诺与店铺政策冲突） |
...
1. **客服回复**：整篇展示 → 等确认 → 用户自己发 → 回写记录到 Base
2. **改 Base**：用 lark-base diff 预览 → 等确认 → 落盘
3. **不替用户发 Etsy 消息**：本 skill 只产文案 + 维护 Base；发消息是用户在 Etsy 后台手动操作
4. **客户隐私**：邮箱、地址、订单号属敏感数据 — 不要在 Agent 输出里大段重复，引用时用脱敏写法（如 `订单 #****1234`）

---

## 与 shop-foundation 的回流关系

客服处理过程中如果用户**纠正**了 Agent 的回复（"语气太冷"、"不该这么快道歉"），按 shop-foundation 的"沉淀模式"提示：
...
- 与用户对话：中文
- 客服回复（发给海外买家）：**英文**
- Base 字段标签：中文 + 英文混用，参考 schema 文件
