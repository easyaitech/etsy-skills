...
- **客户端用 curl（按上面模板）。** 站点前有边缘防护；Python `urllib` / `requests` 的默认 User-Agent 已确认会被它拦下，被拦的请求根本到不了后端。必须用 Python HTTP 库时，只使用文档明确提供且已验证可用的 User-Agent；没有已验证值就改用 curl 模板，不要猜。
...
- ❌ 返回的错误 body **不是 JSON**（纯文本 / HTML 页）→ 没有拿到符合本后端约定的响应，不能据此判断后端状态、权限或具体拦截原因。先按 curl 模板重发；仍非 JSON 时，只如实报告用的地址、客户端和响应形态，交管理员定位。
