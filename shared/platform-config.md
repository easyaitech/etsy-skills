...
- 商品上架 / 商品 Base：`listing-catalog/references/xiaohongshu-commerce.md`
- 订单 / 客服 / 售后 Base：`orders-customers/references/xiaohongshu-orders.md`
...
小红书商品按 SPU / SPL / SPV / ITEM 分层，商品 Base 必须保留品牌、末级类目、规格、图片、描述、产品参数、价格、库存、上下架和审核状态等字段。小红书订单按订单 / 包裹 / SKU / 售后分层，订单 Base 必须保留包裹 ID、订单状态、售后状态、取消状态、SKU 明细、物流模式、承诺发货时间和收件地区等字段。
...
3. 用 `listing-catalog` 建或更新商品 Base，字段用通用名；平台专属字段放 `平台字段 JSON` 或清晰命名的补充字段。
4. 若平台涉及订单或客服，再用 `orders-customers` 建或更新订单 / 客户 Base，字段用通用名 + 平台专属字段分组。
