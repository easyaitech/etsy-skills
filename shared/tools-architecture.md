...
- ✅ 发布 / 写库 / 对外动作有显式确认门（与 [`shared/ethos.md`](ethos.md) §授权才能发布一致）。
- ❌ 不在 skill 里硬编码密钥 / token / 账号密码。
- ❌ 不替用户登录任何外部服务（登录由用户在租户浏览器 / profile 里人工完成）。
- ❌ 不把工具本体、队列、状态机塞进 skill 目录或工作区——通用能力走 git 提拔，租户定制走工作区 / skill-prefs。
...
| `pinterest-autopin` 发布 | Mac mini 上的 Playwright + Chrome profile CLI | tier 2（租户浏览器 + ECS 编排）或 tier 3（反检测专用机），控制面上 ECS | 队列 / 状态 / 身份逐步上收 ECS；登录仍由用户人工完成、token 不进文件（已合规） |
...
| 关注点 | 看这里 |
|---|---|
| skill 目录只读、客户定制走工作区、通用能力走 git | [`shared/preamble.md`](preamble.md) §技能目录写入禁令 |
| 每客户 skill 行为微调（不 fork、不改本体、升级不冲突） | [`shared/skill-prefs.md`](skill-prefs.md) |
...
| 授权才能发布、事实不可自编 | [`shared/ethos.md`](ethos.md) |
