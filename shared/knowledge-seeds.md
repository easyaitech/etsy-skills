...
租户 workspace（<workspace>/，每客户隔离）
  └ BRAND/SHOP/COMMERCE_PLATFORM + skill-prefs/ + knowledge/（含拷入的种子）+ Base
```

`skill-prefs.md` 里的「基座文件 / 旋钮 / 知识」都在**租户个性化层内部**，不是独立架构层。
...
租户自己的文件互相冲突（BRAND vs 某旋钮 vs 某知识卡）→ 软默认序 `品牌真相 > 旋钮 > 知识(evidence)`，由租户裁决。
